...
def 添加通用字段(参数解析器: argparse.ArgumentParser) -> None:
    ...
    参数解析器.add_argument("--下一步日期", help="支持格式：2026-03-27 / 2026/03/27 / 20260327 / 今天 / tomorrow")
...
def 构建参数解析器() -> argparse.ArgumentParser:
    ...
    跟进解析器.add_argument("--预约时间", help="支持：2026-03-27 15:30 / 2026/03/27 15:30 / 今天 15:30")
    跟进解析器.add_argument("--当前阶段")
    跟进解析器.add_argument("--优先级")
    跟进解析器.add_argument("--下一步动作")
    跟进解析器.add_argument("--下一步日期", help="支持多种日期格式")
    跟进解析器.add_argument("--上次沟通摘要")
    跟进解析器.add_argument("--客户想法")

    列表解析器 = 子命令解析器.add_parser("列表", aliases=["机会列表"], help="查看合作机会列表")
    列表解析器.add_argument("--搜索关键词")
    列表解析器.add_argument("--当前阶段")
    列表解析器.add_argument("--优先级")
    列表解析器.add_argument("--内部负责人")
    列表解析器.add_argument("--下一步日期", help="支持多种日期格式")
    列表解析器.add_argument("--下一步日期截止前", help="支持多种日期格式")
...
def 主程序() -> None:
    ...
    try:
        ...
        if 参数.命令 == "跟进":
            请求体 = 构建请求体(参数, ["记录内容", "跟进结果", "沟通方式", "预约时间", "当前阶段", "优先级", "下一步动作", "下一步日期", "上次沟通摘要", "客户想法"])
